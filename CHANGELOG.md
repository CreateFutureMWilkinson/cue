...
- **Fairy flies outside jar bounds** — The fairy's normalized 0.0–1.0 position was mapped to the full container, ignoring `ImageFillContain` letterboxing and the jar's glass walls. Now computes the jar's rendered rect (accounting for pillarboxing/letterboxing) and maps the fairy's position into the jar's interior region (below the lid, above the base, inside the walls). Circle edges are inset from boundaries so the entire body stays within the glass. All animators benefit automatically. (Phase-5-Feature-051)
