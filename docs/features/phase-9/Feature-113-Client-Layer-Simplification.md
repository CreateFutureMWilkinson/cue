...
4. **Remove duplicate WebSocket auth path in `internal/server/handler/websocket.go`.** During the post-107 debug pass we switched the SDK to authenticate the WebSocket upgrade via the standard `Authorization: Bearer …` header so the global `AuthMiddleware` handles it the same way as every other endpoint, and we unwired the handler-level `SetTokenValidator` from `internal/server/server.go`. The `TokenValidator` field, the `SetTokenValidator` method, the `?token=` query check inside `Handler()`, and the three tests that still exercise that code path (`TestWebSocketRejectsInvalidToken`, `TestWebSocketAcceptsValidToken`, `TestWebSocketNoValidatorAllowsAll` in `websocket_test.go`) are now dead code. Delete them as part of the transport pass — the surviving auth surface is the single header-based check in `AuthMiddleware`.
